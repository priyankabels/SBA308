...
function validateCourseId(course, assignment) {
  if (course.id !== assignment.course_id) {
    throw new Error(
      `Invalid data :Assignment group with course Id ${assignment.course_id} does not belong to Course Id ${course.id}. Please correct`
    );
  }
}
//The below function validates data like points possible
function validateData(assignmentObj,submissionObj) {
  ...
  assignmentObj.assignments.forEach((assignment) => {
    if (
      ...
      throw new Error(
        `Invalid Data: Assignment with Id ${assignment.id} has 0 possible points.Please correct.`
      );
    } else if (!assignment.due_at && Date(assignment.due_at)) {
      //check if assignment due date is set
      throw new Error(
        `Invalid Data: Assignment with Id ${assignment.id} has invalid due date.Please correct.`
      );
    }
  });
  //Checking submitted date to be valid if there is valid submission score
  for (let sub of submissionObj)
  {
    if(!(new Date(sub.submission.submitted_at)) || sub.submission.submitted_at==null)
    {
      throw new Error(`Invalid Submitted AT Date ${sub.submission.submitted_at} Please correct`);
    }
    else if(!Number(sub.submission.score))
    {
      throw new Error(`Submission score is invalid`);
    }
  }
...
function filterAssignments(assignments) {
  ...
  const validAssignments = assignments.filter(
    (assignment) => new Date(assignment.due_at) <= new Date()
  );
...
function calculateWeightedPercentage(scores, assignments) {
  ...
  assignments.forEach((assignment) => {
    if (scores[assignment.id] !== undefined) {
      ...
      weightedScore +=
        (scores[assignment.id] / 100) * assignment.points_possible;
      //console.log(weightedScore)
    } else {
      throw new error(
        "No matching submitted assignments found with assignment group assignments Id"
      );
    }
  });
...
function getLearnerData(course, ag, submissions) {
  ...
  //Call  function calculateWeightedPercentage to get results which includes average
  try {
    ...
    for (const learner of Object.values(learners)) {
      ...
      const average = calculateWeightedPercentage(
        learnerscores,
        validAssignments
      );
    ...
    console.error(`Error processing result data: ${error.message}`);
